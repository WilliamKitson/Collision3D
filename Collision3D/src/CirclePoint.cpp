...
{
	if (squareRoot(distance(circle, point)) > absolute(circle[3]))
	{
		return false;
	}
...
float collision3D::CirclePoint::square(float input)
{
	return input * input;
}
...
float collision3D::CirclePoint::absolute(float input)
{
	if (input < 0.0f)
	{
		return -input;
